...
from appyratus.const import (
    ROOT_ATTR,
    OP_DELTA_ADD,
    OP_DELTA_REMOVE,
    OP_DELTA_REPLACE,
)
from appyratus.patch import (
...
def test_JsonPatchMixin_build_match_context_1(mock_bizobj):
    ...
    _test_JsonPatchMixin_build_match_context(mock_bizobj, path,
                                             exp_bizobj_list, exp_objs)
...
def test_JsonPatchMixin_build_match_context_6(mock_bizobj):
    for i in range(len(mock_bizobj.a['d'])):
        path = '/a/d/{}'.format(i)
        exp_objs = [
            mock_bizobj, mock_bizobj.a, mock_bizobj.a['d'],
            mock_bizobj.a['d'][i]
        ]
        exp_bizobj_list = [(0, mock_bizobj)]
        _test_JsonPatchMixin_build_match_context(mock_bizobj, path,
...
def _test_JsonPatchMixin_build_match_context(mock_bizobj, path,
                                             exp_bizobj_list, exp_objs):
